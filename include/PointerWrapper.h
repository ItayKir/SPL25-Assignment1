...
public:
    ...
    T& operator*() const {
        if(ptr == nullptr){
            throw std::runtime_error("[ERROR]: ptr is null, cannot dereference it.");
        }
        return *ptr;
    };
    ...
    T* operator->() const {
        if(ptr == nullptr){
            throw std::runtime_error("[ERROR]: ptr is null, cannot perform arrow operation on it.");
        }
        return ptr;
    }
    ...
    T* get() const {
        if(ptr == nullptr){
            throw std::runtime_error("[ERROR]: ptr is null, cannot perform get() on it.");
        }
        return ptr;
    }
